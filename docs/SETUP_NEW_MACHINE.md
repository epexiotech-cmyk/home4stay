...
## 9. Prisma Setup
Install project dependencies and generate the Prisma client. Ensure `npm` doesn't block the generation script:
```bash
npm config set ignore-scripts false
npm install
cd apps/main-site && npx prisma generate && cd ../..
```
...
- **Prisma Client not found**: Run `cd apps/main-site && npx prisma generate`. Ensure you do not have conflicting `.env` files in the `prisma/` folder.
