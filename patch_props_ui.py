...
"""
if "import RoomForm" not in content:
    content = content.replace('import Link from "next/link";', 'import Link from "next/link";\n' + imports_to_add)

# Inject state in PropertiesPage
...
if "setEditingRoomId" not in content:
    content = content.replace(old_state, new_state)

# Add fetchRooms reference so RoomTypeCard can trigger refresh
# Actually it's easier to just call window.location.reload() or re-fetch activeProperty.
# We'll create a refresh function inside PropertiesPage.
old_fetch = """    const fetchProperty = async () => {"""
new_fetch = """  const fetchProperty = async () => {"""
if "  const fetchProperty = async () => {" not in content:
    # Actually wait, `fetchProperty` is inside `useEffect`.
    # Let's extract it.
    ...
    new_effect = """  const fetchProperty = async () => {
    if (!user?.propertyId) return;
"""
    content = content.replace(old_effect, new_effect)
    
    # We must also update useEffect to call it
    old_end_effect = """    };
    fetchProperty();
  }, [user]);"""
    new_end_effect = """  };
  ...
  useEffect(() => {
    ...
    content = content.replace(old_end_effect, new_end_effect)

# Update RoomTypeCard mapping
...
# Rewrite RoomTypeCard component
old_card = """function RoomTypeCard({ type }: { type: RoomType }) {
  return (
    <GlassCard className="p-6 flex flex-col h-full border-none shadow-premium bg-white/40 dark:bg-white/5">
      <div className="flex justify-between items-start mb-6">
        <div>
          <p className="text-[10px] font-black text-[#0E5A75]/50 uppercase tracking-widest mb-1 opacity-50 truncate max-w-[150px]">{type.id}</p>
      ...
      <div className="space-y-4 flex-1">
        <div className="flex justify-between items-center pb-3 border-b border-black/5 dark:border-white/5">
          ...
          <span className="text-sm font-black text-[#053344] dark:text-white">{type.roomCount} Rooms</span>
        </div>
        <div className="flex justify-between items-center pb-3 border-b border-black/5 dark:border-white/5">
        ...
        <div className="flex flex-wrap gap-1.5 pt-2">
          {type.tags && type.tags.slice(0, 5).map(tag => (
      ...
      <button 
        disabled
        className="mt-8 opacity-50 cursor-not-allowed w-full py-3 rounded-xl border border-[#0E5A75]/20 text-[#0E5A75] text-[10px] font-black uppercase tracking-[0.2em]"
      >
...
new_card = """function RoomTypeCard({ 
  type, 
  editingRoomId, 
  setEditingRoomId, 
  actionLoadingId, 
...
print("Patched RoomTypeCard logic and UI")
